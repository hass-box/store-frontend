...
import { groupBy } from "../../homeassistant-frontend/src/common/util/group-by";
...
import { loadVirtualizer } from "../../homeassistant-frontend/src/resources/virtualizer";
...
import {
  filterData,
  sortData,
} from "../../homeassistant-frontend/src/components/data-table/sort-filter";
...
@customElement("hassbox-data-table")
export class HassBoxDataTable extends LitElement {
            ...
            <slot name="header-row">
              ...
              ${Object.entries(this.columns).map(([key, column]) => {
                ...
                const classes = {
                  ...
                  "mdc-data-table__header-cell--overflow": column.type === "overflow",
  ...
  private _renderRow = (row: DataTableRowData, index: number) => {
    ...
    return html`
      ...
      >
        ${this.selectable
          ? html`
              <div class="mdc-data-table__cell mdc-data-table__cell--checkbox" role="cell">
                <ha-checkbox
                  class="mdc-data-table__row-checkbox"
        ...
        Object.entries(sorted).forEach(([groupName, rows]) => {
          groupedItems.push({
            ...
            >
              <ha-icon-button
                .path=${mdiChevronUp}
                class=${this._collapsedGroups.includes(groupName) ? "collapsed" : ""}
              >
              </ha-icon-button>
  ...
  private _handleRowClick = (ev: Event) => {
    if (
      ev
        .composedPath()
        .find((el) =>
          ["ha-checkbox", "mwc-button", "ha-button", "ha-icon-button", "ha-assist-chip"].includes(
  ...
  private _collapseGroup = (ev: Event) => {
    ...
    if (this._collapsedGroups.includes(groupName)) {
      this._collapsedGroups = this._collapsedGroups.filter((grp) => grp !== groupName);
    } else {
...
declare global {
  interface HTMLElementTagNameMap {
    "hassbox-data-table": HassBoxDataTable;
  }
